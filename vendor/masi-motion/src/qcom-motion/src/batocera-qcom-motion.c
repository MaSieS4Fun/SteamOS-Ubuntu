...
#define READY_FILE "/run/masi/qcom-motion.ready"
#define THOR_CALIBRATION_FILE "/var/lib/masi/qcom-sensors/thor/motion-calibration.ini"
#define ODIN2_CALIBRATION_FILE "/var/lib/masi/qcom-sensors/odin2/motion-calibration.ini"
#define THOR_ACCEL_CALIBRATION_FRAME "thor-dsu-v2"
/* v9: proper 180° about pitch from working (X,Z,Y) — flip yaw+roll, keep chirality. */
#define ODIN2_ACCEL_CALIBRATION_FRAME "odin2-dsu-v9"
...
typedef struct {
	...
	/* Desktop settings ([user] in motion-calibration.ini) */
	gdouble gyro_sensitivity;
	gdouble accel_sensitivity;
	gboolean invert_gyro_horizontal;
	gboolean invert_gyro_vertical;
	gboolean invert_accel_horizontal;
	gboolean invert_accel_vertical;
} MotionServer;
...
open_evdev_motion_device (MotionServer *server, GError **error)
{
	...
	(void) ioctl (fd, UI_SET_PHYS, "qcom-motion/input0");
			   ...
			   gpointer user_data)
{
	...
	if (server->raw_axes) {
		...
		/*
		 * Map each stock SSC stream into one common right-handed frame.
		 * Odin 2: (X,Z,Y) moves (pitch OK) but yaw L/R mirrored. Negating only
		 * yaw mirrors the frame and Steam fusion fights. Fix L/R with a proper
		 * 180° about pitch: (X, -Z, -Y) — same signs on accel and gyro.
		 */
		if (server->profile == MOTION_PROFILE_ODIN2) {
			sample[0] = x / EARTH_GRAVITY;    /* pitch */
			sample[1] = -z / EARTH_GRAVITY;   /* yaw */
			sample[2] = -y / EARTH_GRAVITY;   /* roll (must flip with yaw) */
		} else {
	...
	if (server->invert_accel_horizontal)
		server->accel[0] = -server->accel[0];
	if (server->invert_accel_vertical)
		server->accel[1] = -server->accel[1];
	if (server->accel_sensitivity != 1.0) {
		server->accel[0] *= (float) server->accel_sensitivity;
		server->accel[1] *= (float) server->accel_sensitivity;
		server->accel[2] *= (float) server->accel_sensitivity;
	}
...
transform_gyro (MotionServer *server, const float raw[3], float output[3])
{
	...
	if (server->raw_axes) {
		...
		if (server->profile == MOTION_PROFILE_ODIN2) {
			/*
			 * Same right-handed frame as accel: (X, -Z, -Y).
			 */
			output[0] = corrected[0] * RAD_TO_DEG;
	...
	if (server->invert_gyro_horizontal)
		output[0] = -output[0];
	if (server->invert_gyro_vertical)
		output[1] = -output[1];
	if (server->gyro_sensitivity != 1.0) {
		output[0] *= (float) server->gyro_sensitivity;
		output[1] *= (float) server->gyro_sensitivity;
		output[2] *= (float) server->gyro_sensitivity;
	}
}
...
static void
load_user_motion_settings (MotionServer *server)
{
	GKeyFile *key_file;
	GError *error = NULL;
	double sens;
	...
	if (server->calibration_file == NULL ||
	    !g_file_test (server->calibration_file, G_FILE_TEST_IS_REGULAR))
		return;
	...
	if (!g_key_file_has_group (key_file, "user")) {
		g_key_file_unref (key_file);
		return;
	}

	sens = g_key_file_get_double (key_file, "user", "gyro_sensitivity", &error);
	if (error == NULL && isfinite (sens) && sens > 0.05 && sens <= 10.0)
		server->gyro_sensitivity = sens;
	g_clear_error (&error);

	sens = g_key_file_get_double (key_file, "user", "accel_sensitivity", &error);
	if (error == NULL && isfinite (sens) && sens > 0.05 && sens <= 10.0)
		server->accel_sensitivity = sens;
	g_clear_error (&error);

	if (g_key_file_has_key (key_file, "user", "invert_gyro_horizontal", NULL))
		server->invert_gyro_horizontal =
			g_key_file_get_boolean (key_file, "user", "invert_gyro_horizontal", NULL);
	else if (g_key_file_has_key (key_file, "user", "invert_horizontal", NULL))
		server->invert_gyro_horizontal =
			g_key_file_get_boolean (key_file, "user", "invert_horizontal", NULL);

	if (g_key_file_has_key (key_file, "user", "invert_gyro_vertical", NULL))
		server->invert_gyro_vertical =
			g_key_file_get_boolean (key_file, "user", "invert_gyro_vertical", NULL);
	else if (g_key_file_has_key (key_file, "user", "invert_vertical", NULL))
		server->invert_gyro_vertical =
			g_key_file_get_boolean (key_file, "user", "invert_vertical", NULL);

	if (g_key_file_has_key (key_file, "user", "invert_accel_horizontal", NULL))
		server->invert_accel_horizontal =
			g_key_file_get_boolean (key_file, "user", "invert_accel_horizontal", NULL);
	else if (g_key_file_has_key (key_file, "user", "invert_horizontal", NULL))
		server->invert_accel_horizontal =
			g_key_file_get_boolean (key_file, "user", "invert_horizontal", NULL);

	if (g_key_file_has_key (key_file, "user", "invert_accel_vertical", NULL))
		server->invert_accel_vertical =
			g_key_file_get_boolean (key_file, "user", "invert_accel_vertical", NULL);
	else if (g_key_file_has_key (key_file, "user", "invert_vertical", NULL))
		server->invert_accel_vertical =
			g_key_file_get_boolean (key_file, "user", "invert_vertical", NULL);

	g_message ("Loaded user motion settings: gyro_sens=%.2f accel_sens=%.2f "
		   "gyro_inv_h=%d gyro_inv_v=%d accel_inv_h=%d accel_inv_v=%d",
		   server->gyro_sensitivity, server->accel_sensitivity,
		   server->invert_gyro_horizontal, server->invert_gyro_vertical,
		   server->invert_accel_horizontal, server->invert_accel_vertical);
	g_key_file_unref (key_file);
}
...
main (int argc, char **argv)
{
	...
	server.gyro_sensitivity = 1.0;
	server.accel_sensitivity = 1.0;
	server.invert_gyro_horizontal = FALSE;
	server.invert_gyro_vertical = FALSE;
	server.invert_accel_horizontal = FALSE;
	server.invert_accel_vertical = FALSE;
	set_identity_accel_calibration (&server);
	...
	if (!calibrate) {
		...
		load_user_motion_settings (&server);
	} else {
