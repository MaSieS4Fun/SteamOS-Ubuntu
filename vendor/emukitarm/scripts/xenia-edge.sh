#!/usr/bin/env bash
# Build / install / uninstall Xenia Edge from source via upstream ./xb.
#
# Edge is a Canary fork focused on faster Vulkan/Linux iteration. No cmake
# --install on Linux: we install the Release binary + desktop/icon under
# /usr/local (manifest-backed uninstall).
#
# Requires ./xb slang (downloads slangc for aarch64) before setup.
# Linux/ARM64 support is experimental. Do not pass -mcpu=native.
set -euo pipefail
...
APP_ID="xenia-edge"
APP_NAME="Xenia Edge"
REPO_URL="https://github.com/has207/xenia-edge.git"
REPO_BRANCH="edge"
BIN_NAME="xenia_edge"
...
BUILD_DEPS=(
  ...
  liblz4-dev libvulkan-dev libx11-xcb-dev
  libfontconfig1-dev
...
# wxWidgets is vendored; GTK may still be pulled for dialogs/portal helpers.
...
do_install() {
  ...
  masi_log "=== Installing ${APP_NAME} (./xb slang + setup + build, manual install) ==="
  ...
  masi_log "Running ./xb slang (download slangc for aarch64)..."
  masi_xenia_run_xb "$src_dir" slang

  masi_log "Running ./xb setup (submodules + cmake)..."
  masi_xenia_run_xb "$src_dir" setup

  masi_log "Running ./xb build --config=release..."
  ...
  masi_xenia_system_install \
    ...
    "$BIN_NAME" \
    "$APP_NAME" \
    "Xbox 360 research emulator (Edge / Vulkan-Linux focus)" \
    "$binary" \
    "${src_dir}/assets/xenia_edge.desktop" \
